...
public:
  ...
  void processPayloadChannelNotifications(std::string event,const json & data) 
  {
	...
	{
		this->_channel->on(this->_internal["transportId"],[self = Transport::downcasted_shared_from_this<PlainTransport>()]( std::string event,json data ) //this->_internal.transportId, (event, data?: any) =>
		{		 
