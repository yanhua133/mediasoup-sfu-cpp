...
PayloadChannelAgent::PayloadChannelAgent(ChannelObserver* obs) {
	MS_lOGF();
...
PayloadChannelAgent::~PayloadChannelAgent() {
	MS_lOGF();
    this->_closed = true;
...
bool PayloadChannelAgent::Init(int producerFd,int consumerFd) {
	MS_lOGF();
    MS_lOGE("PayloadChannelAgent::Init producerFd=%d consumerFd=%d",producerFd, consumerFd);
...
bool PayloadChannelAgent::Start() {
	MS_lOGF();
	MS_ASSERT_RV_LOGE(m_consumerPipe->Start(), false, "m_consumerPipe start failed");
	return true;
}
void PayloadChannelAgent::Write(const uint8_t* data, size_t len)
{
  m_producerPipe->Write(data,len);
}

void PayloadChannelAgent::SetPid(int pid) {
	m_pid = pid;
}
void PayloadChannelAgent::removeAllListeners(std::string producerId)
{
    
}
json PayloadChannelAgent::request(const char * method, json& internal)
{
...
UVPipeWrapper * PayloadChannelAgent::GetConsumer() {
	return m_consumerPipe.get();
}


void PayloadChannelAgent::OnRead(UVPipeWrapper * pipe, uint8_t * data, size_t len) {
    MS_lOGD("PayloadChannelAgent::OnRead: rawdata = %s", (char*)data);
    MS_lOGD("PayloadChannelAgent::OnRead:m_readBuf left =%s", m_readBuf.c_str());
    m_readBuf.append((char*)data, len);
	MS_lOGD("PayloadChannelAgent::OnRead:m_readBuf append =%s", m_readBuf.c_str());
...
void PayloadChannelAgent::processMessage(const nlohmann::json& msg) {
    MS_lOGD("PayloadChannel::processMessage: msg=%s",msg.dump().c_str());
    if(msg.contains("id")) 
    {
...
void PayloadChannelAgent::OnClose(UVPipeWrapper * pipe) {
	if (pipe == m_consumerPipe.get()) {
		MS_lOGI("m_consumerPipe PayloadChannel OnClose");
	}

	if (pipe == m_producerPipe.get()) {
		MS_lOGI("m_producerPipe PayloadChannel OnClose");
	}
