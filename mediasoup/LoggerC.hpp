 ...
 * source file including LoggerC.hpp MUST define its own MSC_CLASS macro. Include
 * files (.hpp) MUST NOT include LoggerC.hpp.
 *
...
using LoggerC = mediasoupclient::LoggerC;
...
#define _MSC_LOG_SEPARATOR_CHAR "\n"
	...
	#define MSC_TRACE() \
		do \
		{ \
			if (LoggerC::handler && LoggerC::logLevel == LoggerC::LogLevel::LOG_DEBUG) \
			{ \
				int loggerWritten = std::snprintf(LoggerC::buffer, LoggerC::bufferSize, "[TRACE]" _MSC_LOG_STR, _MSC_LOG_ARG); \
				LoggerC::handler->OnLog(LoggerC::LogLevel::LOG_TRACE, LoggerC::buffer, loggerWritten); \
			} \
		} \
		while (false)
#else
...
#define MSC_DEBUG(desc, ...) \
	...
	{ \
		if (LoggerC::handler && LoggerC::logLevel == LoggerC::LogLevel::LOG_DEBUG) \
		{ \
			int loggerWritten = std::snprintf(LoggerC::buffer, LoggerC::bufferSize, "[DEBUG]" _MSC_LOG_STR_DESC desc, _MSC_LOG_ARG, ##__VA_ARGS__); \
			LoggerC::handler->OnLog(LoggerC::LogLevel::LOG_DEBUG, LoggerC::buffer, loggerWritten); \
		} \
	} \
	while (false)
...
#define MSC_WARN(desc, ...) \
	do \
	{ \
		if (LoggerC::handler && LoggerC::logLevel >= LoggerC::LogLevel::LOG_WARN) \
		{ \
			int loggerWritten = std::snprintf(LoggerC::buffer, LoggerC::bufferSize, "[WARN]" _MSC_LOG_STR_DESC desc, _MSC_LOG_ARG, ##__VA_ARGS__); \
			LoggerC::handler->OnLog(LoggerC::LogLevel::LOG_WARN, LoggerC::buffer, loggerWritten); \
		} \
	} \
	while (false)
...
#define MSC_ERROR(desc, ...) \
	do \
	{ \
		if (LoggerC::handler && LoggerC::logLevel >= LoggerC::LogLevel::LOG_ERROR) \
		{ \
			int loggerWritten = std::snprintf(LoggerC::buffer, LoggerC::bufferSize, "[ERROR]" _MSC_LOG_STR_DESC desc, _MSC_LOG_ARG, ##__VA_ARGS__); \
			LoggerC::handler->OnLog(LoggerC::LogLevel::LOG_ERROR, LoggerC::buffer, loggerWritten); \
		} \
	} \
	while (false)
