	...
	namespace Sdp
	{
		namespace Utils
		{
			...
			void addLegacySimulcast(json& offerMediaObject, uint8_t numStreams)
			{
				...
				auto v = mediasoupclient::UtilsC::split(ssrcMsidLine["value"].get<std::string>(), ' ');
				auto& streamId = v[0];
				auto& trackId  = v[1];
				...
				if (jsonSsrcGroupsIt != offerMediaObject.end())
				{
					...
					auto ret = std::find_if(
					  ssrcGroups.begin(), ssrcGroups.end(), [&firstSsrc, &firstRtxSsrc](const json& line) {
						  ...
						  auto v = mediasoupclient::UtilsC::split(jsonSsrcsIt->get<std::string>(), ' ');

						  if (std::stoull(v[0]) == firstSsrc)
						  {
							  firstRtxSsrc = (uint32_t)std::stoull(v[1]);
				...
				auto ssrcsLine               = mediasoupclient::UtilsC::join(ussrcs, ' ');
			...
			json getRtpEncodings(const json& offerMediaObject)
			{
				...
				if (jsonSsrcGroupsIt != offerMediaObject.end())
				{
					...
					for (const auto& line : ssrcGroups)
					{
						...
						auto v       = mediasoupclient::UtilsC::split(fidLine, ' ');
						auto ssrc    = std::stoull(v[0]);
						auto rtxSsrc = std::stoull(v[1]);

						// Remove the RTX SSRC from the List so later we know that they
						// are already handled.
						ssrcs.remove((const uint32_t)rtxSsrc);

						// Add to the map.
						ssrcToRtxSsrc[(const unsigned int)ssrc] = (uint32_t)rtxSsrc;
					}
				}
